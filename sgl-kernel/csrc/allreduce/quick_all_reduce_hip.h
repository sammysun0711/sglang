...
template <typename AllReduceKernel, typename T>
__global__ __quickreduce_launch_bounds_mimo_rmsnorm__ static void allreduce_mimo_rmsnorm_prototype_twoshot(
    T const* A,
...
#define TWOSHOT_MIMO_RMSNORM_DISPATCH()                                                       \
  if (world_size == 2) {                                                                      \
    ...
    using AllReduceKernel = AllReduceTwoshotMimoRMSNorm<T, CommT, LineCodec, cast_bf2half>;   \
    hipLaunchKernelGGL(                                                                       \
  ...
  void allreduce_mimo_rmsnorm(
    ...
    if (hidden_dim != kMimoRMSNormHiddenDim) {
      throw std::runtime_error(
          "MiMo QR RMSNorm fusion requires hidden_dim = " + std::to_string(kMimoRMSNormHiddenDim));
    }
    if (N % hidden_dim != 0) {
      throw std::runtime_error("MiMo QR RMSNorm fusion requires numel divisible by hidden_dim");
    }
