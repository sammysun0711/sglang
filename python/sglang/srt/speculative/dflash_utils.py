...
def get_dflash_attention_sliding_window_size(config: Any) -> Optional[int]:
    ...
    if draft_config.use_swa:
        sliding_window = draft_config.swa_window_size
        if sliding_window is None:
            text_config = _get_text_config(config)
            sliding_window = _parse_optional_int(
                _cfg_get(
                    text_config,
                    "sliding_window",
                    _cfg_get(config, "sliding_window"),
                ),
                field_name="DFLASH sliding_window",
                min_value=1,
            )
        if sliding_window is None:
            raise ValueError(
                "DFLASH dflash_config.use_swa requires "
                "dflash_config.swa_window_size or config.sliding_window."
            )
...
def parse_dflash_draft_config(*, draft_hf_config: Any) -> DFlashDraftConfig:
    ...
    if raw_backbone_rotary_base is None:
        ...
        if not math.isfinite(backbone_rotary_base) or backbone_rotary_base <= 0:
            raise ValueError(
                "DFLASH dflash_config.backbone_rotary_base must be finite and "
                f"positive, got {backbone_rotary_base}."
            )
