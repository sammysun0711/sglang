...
- `--gsp-question-len`
- `--gsp-output-len`
- `--gsp-prewarm-prefixes`: flush the cache, then populate every generated prefix group before measured traffic. Priming requests are excluded from benchmark metrics. Use an SGLang backend.
- `--gsp-prewarm-concurrency`: maximum concurrent priming requests (default: `1`).
- Prewarmed GSP workloads use deterministic cyclic source tokens and isolate the first source question token within each active group. Text-mode expectations use the locally re-tokenized prefix/full-prompt common length and report a page-aligned expectation when the server exposes its cache page size.
